...
class AvatarService:
    ...
    def _download_selfie_bytes(self, user_id: str) -> bytes:
        """Download the user's selfie from storage.

        Tries common extensions for the `{user_id}` naming convention.
        """
        ...
        # If not found with strict user id filename, try a generic selfie filename
        # This mirrors existing conventions where a gender-based default might exist
        try:
            # This will raise if not present; it's just a last-resort attempt
    ...
    def _get_user_metrics(self, user_id: str) -> tuple[Optional[float], Optional[float], Optional[str]]:
    ...
    def _upload_avatar(self, *, user_id: str, image_bytes: bytes) -> str:
        """Upload avatar bytes and return public URL."""
        mime_type, ext = detect_image_mime_and_ext(image_bytes)
        filename = f"{user_id}{ext}"
        storage = self.supabase.storage.from_(self.AVATAR_BUCKET)

        # Upsert-like behavior: if file exists, overwrite
        # Some storage drivers use `upsert=True` via file_options
    ...
    def generate_and_store_avatar(self, *, user_id: str, selfie_bytes: Optional[bytes] = None) -> Dict[str, str]:
        """End-to-end: selfie -> Gemini -> avatar upload. Returns public URL.

        If `selfie_bytes` is provided, use it directly; otherwise attempt to
