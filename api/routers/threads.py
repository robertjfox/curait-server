...
async def create_thread(request: ThreadCreateRequest):
    ...
    try:
        thread_id = threads_interface.create(user_id=request.user_id)
...
async def list_threads_for_user(user_id: str):
    ...
    try:
        threads = threads_interface.list_summaries_by_user(user_id)
...
async def get_thread(thread_id: str):
    ...
    try:
        thread = threads_interface.get(thread_id)
        if not thread:
...
async def list_outfits_for_thread(thread_id: str):
    ...
    try:
        outfits = outfits_interface.list_for_thread_with_items(thread_id)
...
async def reveal_next_outfit_for_thread(thread_id: str):
    ...
    try:
        result = await outfit_generation_service.reveal_next_cached_outfit(
            thread_id=thread_id,
        )
...
async def delete_thread(thread_id: str):
    ...
    try:
        ok = threads_interface.delete(thread_id)
        if not ok:
